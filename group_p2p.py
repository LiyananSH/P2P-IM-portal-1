...
from websocket import notify_group_message
...
async def receive_group_message(
    ...
    try:
        ...
        current_user = result.scalar_one_or_none()
        if current_user:
            await notify_group_message(current_user.id, {
                "group_id": new_message.group_uuid or new_message.group_id,
                "sender_name": new_message.sender_name,
                "sender_portal": new_message.sender_portal,
                "content": new_message.content,
                "message_id": new_message.id,
                "created_at": new_message.created_at.isoformat() if new_message.created_at else ""
            })
    except Exception as e:
        print(f"Failed to notify group message via WebSocket: {e}")
